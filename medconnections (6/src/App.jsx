...
export default function App() {
  ...
  // Purely presentational: the homepage carries the brand's signature
  // periwinkle canvas (Home visual-identity pass), while every other
  // screen — especially the puzzle board — stays on the app's calmer,
  // near-neutral surface. Toggled on <body> rather than a wrapper div so
  // the color can bleed to the full viewport instead of stopping at the
  // centered app-shell column. No effect on any game logic/state.
  useEffect(() => {
    document.body.classList.toggle('bg-home', view === 'home' && !isDevRoute)
  }, [view, isDevRoute])

  // The 5-Minute Challenge's active round is its own "focused state"
  // (Section 12 of the visual-identity pass): a deeper tone and less
  // surrounding chrome while a round is actually live, back to normal the
  // moment the intro/results screens show. Presentational only.
  useEffect(() => {
    document.body.classList.toggle('bg-challenge-focus', view === 'challenge' && challengePhase === 'playing')
  }, [view, challengePhase])

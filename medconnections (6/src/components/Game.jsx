...
export default function Game({
  ...
  const handleSubmit = () => {
    ...
    if (isFullMatch(selected)) {
      ...
      setGuessLog((prev) => [
        ...prev,
        { levels: [selected[0].level, selected[0].level, selected[0].level, selected[0].level], catIndexes, correct: true, durationMs },
      ])
  ...
  return (
    <div className="game">
      ...
      <div className={`solved-stack ${gameOver ? 'solved-stack-complete' : ''}`}>
        {orderedSolvedCats.map((c) => (
          <div
            ...
            style={{ '--level-color': levelColor(c.level) }}
          >
            <div className="solved-banner-marker" aria-label={levelShapeLabel(c.level)}>
              {levelShape(c.level)}
            </div>
            <div className="solved-banner-body">
            ...
            {remainingTiles.map((tile) => {
              ...
              return (
                <button
                  ...
                  } ${isShaking ? 'tile-shake' : ''} ${isOneAwayPulse ? 'tile-oneaway-pulse' : ''}`}

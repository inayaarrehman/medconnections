...
export function buildShareText({ isDaily, dailyNumber, puzzleTitle, shareLabel, guessLog, won, mistakes, dailyStreak }) {
  ...
  if (isDaily) {
    const header = `Medical Connections · Daily #${String(dailyNumber ?? '').padStart(3, '0')}`
    const missLine = won ? (mistakes === 0 ? 'Perfect connection.' : `${mistakes} miss${mistakes === 1 ? '' : 'es'}`) : 'Not solved today'
    const streakLine = won && dailyStreak > 0 ? `\n${dailyStreak} day streak` : ''
    return `${header}\n${rows}\n${missLine}${streakLine}`
  }

  const header = shareLabel ? `Medical Connections · ${shareLabel}` : `Medical Connections · ${puzzleTitle}`
  const missLine = won ? (mistakes === 0 ? 'Perfect connection.' : `${mistakes} miss${mistakes === 1 ? '' : 'es'}`) : 'Not solved'
  return `${header}\n${rows}\n${missLine}`
}

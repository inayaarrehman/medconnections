...
const MASTERY_BIAS = { unseen: 3, seen: 2, learning: 1, mastered: 0 }

function categoryMasteryWeight(category, mastery) {
  const state = mastery[category.id]?.state || 'unseen'
  return MASTERY_BIAS[state] ?? 2
}

// One pool per difficulty tier for a system: categories tagged with that
// system first; if a tier has none, fall back to the full verified pool
// for that tier so a thin system can still produce a complete puzzle
// (this is the "carefully selected cross-system categories" allowance).
function tierPools(bank, system) {
  ...
  tiers.forEach((tier) => {
    ...
    const primary = ofTier.filter((c) => c.systems.includes(system))
    pools[tier] = primary.length > 0 ? primary : ofTier
  })
  return pools
}

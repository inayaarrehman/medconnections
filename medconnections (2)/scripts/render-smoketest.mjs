...
import { saveProgress, getDailyHistory, getSystemProgress } from '../src/utils/storage.js'
...
check('Home renders before the daily is done (Study section locked)', () => {
  ...
  if (!html.includes('study-grid locked')) throw new Error('expected "study-grid locked" class when daily not done')
  if (html.includes('continue-banner')) throw new Error('did not expect a Continue Studying banner before the daily is done')
  return html
})

check('Home renders after the daily is done, with streak flame + Continue Studying banner', () => {
  const html = renderToStaticMarkup(
    React.createElement(Home, {
      dailyNumber: 999,
      dailyDone: true,
  ...
  if (html.includes('study-grid locked')) throw new Error('expected Study section unlocked once daily is done')
  if (!html.includes('continue-banner')) throw new Error('expected a Continue Studying banner referencing the last-played system')
  if (!html.includes('Cardiology')) throw new Error('expected the continue banner to name the system')
  if (!html.includes('home-streak')) throw new Error('expected the streak flame to render for a 7-day streak')
  return html
})

check('Home renders exactly 2 Study cards (Archive + Systems) and 2 Coming Soon cards', () => {
  const html = renderToStaticMarkup(
    React.createElement(Home, {
      dailyNumber: 1,
      dailyDone: true,
  ...
  const studyMatches = html.match(/study-card-title/g) || []
  const lockedMatches = html.match(/locked-card/g) || []
  if (studyMatches.length !== 2) throw new Error(`expected 2 study cards (Archive, Systems), found ${studyMatches.length}`)
  if (lockedMatches.length !== 2) throw new Error(`expected 2 coming-soon locked cards, found ${lockedMatches.length}`)
  return html
})
...
check(`Systems renders the system grid (all ${SYSTEMS.length} named systems)`, () => {
  const html = renderToStaticMarkup(
    React.createElement(Systems, {
      systemPuzzles,
      systemProgress: getSystemProgress(),
      onPlayPuzzle: () => {},
      onBack: () => {},
    })
  )
  const tileMatches = html.match(/system-tile-name/g) || []
  if (tileMatches.length !== SYSTEMS.length) {
    throw new Error(`expected ${SYSTEMS.length} system tiles, found ${tileMatches.length}`)
  }
  return html
})

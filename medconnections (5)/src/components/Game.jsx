    ...
    <div className="game">
      ...
      <div className={`solved-stack ${gameOver ? 'solved-stack-complete' : ''}`}>
        {orderedSolvedCats.map((c) => (
          ...
          >
            <div className="solved-banner-title">
              <span className="difficulty-shape" aria-label={levelShapeLabel(c.level)}>
                {levelShape(c.level)}
              </span>
              {c.title}
            </div>
      ...
      {gameOver && (
        <div className={`result-card ${won ? 'result-card-won' : ''} ${isPerfect ? 'result-card-perfect' : ''}`}>
          ...
          {won && <BrandMark size={40} className="result-brandmark" animate />}

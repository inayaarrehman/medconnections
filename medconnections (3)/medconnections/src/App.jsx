...
function buildMasteryResults(puzzle, guessLog) {
  ...
  puzzle.categories.forEach((cat, catIndex) => {
    ...
    results.push({ bankCategoryId: cat.bankCategoryId, cleanSolve: wasCorrectlySolved && !wasWronglyTouched })
  })
  return results
}
...
export default function App() {
  ...
  const continueSystemCounts = useMemo(
    () => (continueSystem ? systemMasteryCounts(connectionBank, continueSystem, mastery) : { total: 0, mastered: 0 }),
  ...
  const playSystem = (system) => {
    ...
    if (!puzzle) return // not enough bank content for this system yet
    setGameCtx({
      puzzle,
      mode: 'system',
  ...
  if (view === 'systems') {
    return (
      <div className="app-shell">
        <Systems bank={connectionBank} mastery={mastery} onPlaySystem={playSystem} onBack={goHome} />
      </div>
    )
  }
  ...
  return (
    <div className="app-shell">
      <Home
        ...
        continueSystemMastered={continueSystemCounts.mastered}

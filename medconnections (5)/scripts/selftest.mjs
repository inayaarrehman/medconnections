...
console.log('\n[13c] Polish-pass features: completion phrases, share text, daily micro-stat, Connection of the Day')
{
  ...
  assert(perfectPhrase === 'Perfect connection.', 'a zero-mistake solve always gets the distinct "Perfect connection." phrase')
  ...
  assert(share.includes('Medical Connections'), 'share text identifies the app by its own name')
...
console.log('\n[14] 5-Minute Challenge: round generation, distractor plausibility, verified-only content')
{
...
console.log('\n[15] 5-Minute Challenge: scoring math')
{
...
console.log('\n[16] 5-Minute Challenge: storage (personal best + history, independent of the daily streak)')
{
  localStorage.clear()
  ...
  assert(
    ...
    'recording a 5-Minute Challenge result never changes the Daily Puzzle streak/stats (no second streak introduced)'
  )
}

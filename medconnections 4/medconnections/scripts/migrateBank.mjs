...
function inferConnectionType(puzzle, category) {
  ...
  if (/stroke|territory|movement disorder/.test(text)) return 'cross-system'
  return 'knowledge'
}
...
systemPuzzles.forEach((puzzle) => {
  puzzle.categories.forEach((category) => {
    const entry = {
      ...
      systems: puzzle.systems && puzzle.systems.length ? puzzle.systems : ['Mixed / Step Review'],
      ...
      overlapTags: [],
      status: 'verified',
      notes: `Migrated from hand-written puzzle "${puzzle.title}" (${puzzle.id}) — already reviewed content, now a standalone bank category so the assembler can recombine it freely.`,
    }
    migrated.push(entry)
  })
})

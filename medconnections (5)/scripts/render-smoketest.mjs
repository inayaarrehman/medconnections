...
check('Home renders a plain-text "Today complete" line + a Continue section after the daily is done', () => {
  ...
  if (!html.includes('2,840')) throw new Error('expected the 5-Minute Challenge personal best to render')
  return html
})

check('Home omits the Continue section with no play history yet, but still offers the 5-Minute Challenge', () => {
  const html = renderToStaticMarkup(
    React.createElement(Home, {
      dailyNumber: 1,
      dailyDone: true,
  ...
  if (!html.includes('5-Minute Challenge')) throw new Error('expected the 5-Minute Challenge section to always be offered')
  return html
})
...
check('AppNav renders all 4 primary nav items with the active one marked', () => {
  ...
  ;['Today', 'Systems', '5-Minute', 'Review'].forEach((label) => {
...
check('Game shows "Perfect connection." (not a rotating phrase) on a zero-mistake win', () => {
  ...
  if (!html.includes('Perfect connection.')) throw new Error('expected the distinct "Perfect connection." phrase on a zero-mistake win')
...
check('Game share text uses the app\'s own shape language, not NYT Connections\' colored-square emoji grid', () => {
  ...
  if (!html.includes('Medical Connections')) throw new Error('expected the share text/aria-label to identify the app by its own name')
  return html
})
